...
def first_not_repeating_char(char_sequence):
    ...
    for ind, letter in enumerate(char_sequence):
        if letter not in seen_letters:
            seen_letters[letter] = (ind,1)
        else:
    ...
    not_repeter_letters = sorted(final_letters, key=lambda value: value[1])

    if not_repeter_letters:
        return not_repeter_letters[0][0]
    else:
        return '_'
